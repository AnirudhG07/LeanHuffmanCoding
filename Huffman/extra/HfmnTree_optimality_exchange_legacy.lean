/-
  ...
  Legacy exchange-based development.
  It is kept for reference but is not used by the active induction-based
  optimality route (`HfmnTree_optimality_core` + `HfmnTree_optimality_induction`
  + `HfmnTree_optimality_final`).
